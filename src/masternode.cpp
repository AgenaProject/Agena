...
//#3
	...
	if (MASTERNODE_PRICE > 999) MASTERNODE_PRICE = 1000 ;
        ...
        int MASTERNODE_PRICE4 = 2000 ;
        int MASTERNODE_PRICE5 = 3250 ;
	...
	int cValue =  coins.vout[nOutputIndex].nValue/COIN;
        ...
        if (coins.nHeight > 90000 && cValue < MASTERNODE_PRICE7){
        ...
        if (coins.nHeight > 70000 && cValue < MASTERNODE_PRICE6){
                strErrorRet = strprintf("Your Masternode output has %d AGENA, and it was made at Block Height %d. You need %d AGENA to start masternode now.", cValue, coins.nHeight, MASTERNODE_PRICE);
                return false;
        }

        if (coins.nHeight > 35000 && cValue < MASTERNODE_PRICE5){
                strErrorRet = strprintf("Your Masternode output has %d AGENA, and it was made at Block Height %d. You need %d AGENA to start masternode now.", cValue, coins.nHeight, MASTERNODE_PRICE);
                return false;
        }
        if (coins.nHeight > 20000 && cValue < MASTERNODE_PRICE4){
                strErrorRet = strprintf("Your Masternode output has %d AGENA, and it was made at Block Height %d. You need %d AGENA to start masternode now.", cValue, coins.nHeight, MASTERNODE_PRICE);
                return false;
        }
        if (coins.nHeight > 9000 && cValue < MASTERNODE_PRICE3){
                strErrorRet = strprintf("Your Masternode output has %d AGENA, and it was made at Block Height %d. You need %d AGENA to start masternode now.", cValue, coins.nHeight, MASTERNODE_PRICE);
                return false;
        }
...
//LogPrintf("CMasternodeBroadcast::CheckOutpoint -- Called");
	...
	if (MASTERNODE_PRICE > 999) MASTERNODE_PRICE = 1000 ;
       ...
       if(coins.vout[vin.prevout.n].nValue !=MASTERNODE_PRICE * COIN && coins.vout[vin.prevout.n].nValue !=MASTERNODE_PRICE1 * COIN  && coins.vout[vin.prevout.n].nValue !=MASTERNODE_PRICE2 * COIN && coins.vout[vin.prevout.n].nValue !=MASTERNODE_PRICE3 * COIN && coins.vout[vin.prevout.n].nValue !=MASTERNODE_PRICE4 * COIN && coins.vout[vin.prevout.n].nValue !=MASTERNODE_PRICE5 * COIN  ) {
            LogPrint("masternode", "CMasternodeBroadcast::CheckOutpoint -- Masternode UTXO should have 1000 AGN, masternode=%s\n", vin.prevout.ToStringShort());
            return false;
        }
    ...
    // should be at least not earlier than block when 1000 AGN tx got nMasternodeMinimumConfirmations
    ...
    GetTransaction(vin.prevout.hash, tx2, Params().GetConsensus(), hashBlock, true);
    {
        ...
        if (mi != mapBlockIndex.end() && (*mi).second) {
            CBlockIndex* pMNIndex = (*mi).second; // block for 1000 AGN tx -> 1 confirmation

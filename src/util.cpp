...
const char * const BITCOIN_CONF_FILENAME = "agena.conf";
const char * const BITCOIN_PID_FILENAME = "agenad.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "agena" is a composite category enabling all Agena-related debug output
            if(ptrCategory->count(string("agena"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\AgenaCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\AgenaCore
    // Mac: ~/Library/Application Support/AgenaCore
    // Unix: ~/.agenacore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "AgenaCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/AgenaCore";
#else
    // Unix
    return pathRet / ".agenacore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty agena.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override agena.conf
